var _self = "undefined" != typeof window ? window : "undefined" != typeof WorkerGlobalScope && self instanceof WorkerGlobalScope ? self : {}, Prism = function () {
    var a = /\blang(?:uage)?-(?!\*)(\w+)\b/i, b = _self.Prism = {
        util: {
            encode: function (k) {
                return k instanceof d ? new d(k.type, b.util.encode(k.content), k.alias) : "Array" === b.util.type(k) ? k.map(b.util.encode) : k.replace(/&/g, "\x26amp;").replace(/</g, "\x26lt;").replace(/\u00a0/g, " ")
            }, type: function (b) {
                return Object.prototype.toString.call(b).match(/\[object (\w+)\]/)[1]
            }, clone: function (a) {
                switch (b.util.type(a)) {
                    case "Object":
                        var h =
                        {}, c;
                        for (c in a)a.hasOwnProperty(c) && (h[c] = b.util.clone(a[c]));
                        return h;
                    case "Array":
            ...
            extend: function (a, h) {
                var c = b.util.clone(b.languages[a]), e;
                for (e in h)c[e] = h[e];
                return c
            }, insertBefore: function (a, h, c, e) {
                e = e || b.languages;
                var d = e[a];
                ...
                for (m in d)if (d.hasOwnProperty(m)) {
                    if (m == h)for (f in c)c.hasOwnProperty(f) && (g[f] = c[f]);
                    g[m] = d[m]
                }
                ...
                for (var e in a)a.hasOwnProperty(e) && (h.call(a, e, a[e], c || e), "Object" === b.util.type(a[e]) ? b.languages.DFS(a[e], h) : "Array" === b.util.type(a[e]) && b.languages.DFS(a[e], h, e))
            }
        }, plugins: {}, highlightAll: function (a, h) {
            for (var c, e = document.querySelectorAll('code[class*\x3d"language-"], [class*\x3d"language-"] code, code[class*\x3d"lang-"], [class*\x3d"lang-"] code'), d = 0; c = e[d++];)b.highlightElement(c,
                !0 === a, h)
        }, highlightElement: function (k, h, c) {
            for (var e, d, f = k; f && !a.test(f.className);)f = f.parentNode;
            f && (e = (f.className.match(a) || [, ""])[1], d = b.languages[e]);
            k.className = k.className.replace(a, "").replace(/\s+/g, " ") + " language-" + e;
            f = k.parentNode;
            /pre/i.test(f.nodeName) && (f.className = f.className.replace(a, "").replace(/\s+/g, " ") + " language-" + e);
            var f = k.textContent, g = {element: k, language: e, grammar: d, code: f};
            if (!f || !d)return void b.hooks.run("complete", g);
            (b.hooks.run("before-highlight", g), h && _self.Worker) ?
                (k = new Worker(b.filename), k.onmessage = function (a) {
                    g.highlightedCode = a.data;
                    b.hooks.run("before-insert", g);
                    g.element.innerHTML = g.highlightedCode;
                    c && c.call(g.element);
                    b.hooks.run("after-highlight", g);
                    b.hooks.run("complete", g)
                }, k.postMessage(JSON.stringify({
                    language: g.language,
                    code: g.code,
                    immediateClose: !0
                }))) : (g.highlightedCode = b.highlight(g.code, g.grammar, g.language), b.hooks.run("before-insert", g), g.element.innerHTML = g.highlightedCode, c && c.call(k), b.hooks.run("after-highlight", g), b.hooks.run("complete",
                g))
        }, highlight: function (a, h, c) {
            a = b.tokenize(a, h);
            return d.stringify(b.util.encode(a), c)
        }, tokenize: function (a, d, c) {
            c = b.Token;
            var e = [a], l = d.rest;
            if (l) {
                for (var f in l)d[f] = l[f];
                delete d.rest
            }
            a:for (f in d)if (d.hasOwnProperty(f) && d[f])for (var l = d[f], l = "Array" === b.util.type(l) ? l : [l], g = 0; g < l.length; ++g)for (var m = l[g], n = m.inside, w = !!m.lookbehind, v = 0, x = m.alias, m = m.pattern || m, u = 0; u < e.length; u++) {
                var q = e[u];
                ...
                if (!(q instanceof c)) {
                    ...
                    if (p) {
                        w && (v = p[1].length);
                        var t = p.index - 1 + v, p = p[0].slice(v), r = t + p.length, t = q.slice(0, t + 1), q = q.slice(r + 1), r = [u, 1];
                        t && r.push(t);
                        p = new c(f, n ? b.tokenize(p, n) : p, x);
                        r.push(p);
                        q && r.push(q);
            ...
            all: {}, add: function (a, d) {
                var c = b.hooks.all;
                c[a] = c[a] || [];
                c[a].push(d)
            }, run: function (a, d) {
                var c = b.hooks.all[a];
                if (c && c.length)for (var e, l = 0; e = c[l++];)e(d)
            }
        }
    }, d = b.Token = function (a, b, c) {
        this.type = a;
        this.content = b;
        this.alias = c
    };
    if (d.stringify = function (a, h, c) {
            if ("string" == typeof a)return a;
            if ("Array" ===
                b.util.type(a))return a.map(function (b) {
                return d.stringify(b, h, a)
            }).join("");
            c = {
                type: a.type,
                content: d.stringify(a.content, h, c),
                tag: "span",
            ...
            if ("comment" == c.type && (c.attributes.spellcheck = "true"), a.alias) {
                var e = "Array" === b.util.type(a.alias) ? a.alias : [a.alias];
                Array.prototype.push.apply(c.classes, e)
            }
            b.hooks.run("wrap", c);
            var e = "", l;
            for (l in c.attributes)e += (e ? " " : "") + l + '\x3d"' + (c.attributes[l] || "") + '"';
            return "\x3c" + c.tag + ' class\x3d"' + c.classes.join(" ") +
                '" ' + e + "\x3e" + c.content + "\x3c/" + c.tag + "\x3e"
        }, !_self.document)return _self.addEventListener ? (_self.addEventListener("message", function (a) {
        a = JSON.parse(a.data);
        var d = a.language, c = a.immediateClose;
        _self.postMessage(b.highlight(a.code, b.languages[d], d));
        c && _self.close()
    }, !1), _self.Prism) : _self.Prism;
    var n = document.getElementsByTagName("script");
    return n = n[n.length - 1], n && (b.filename = n.src, document.addEventListener && !n.hasAttribute("data-manual") && document.addEventListener("DOMContentLoaded", b.highlightAll)),
        _self.Prism
}();
...
Prism.hooks.add("wrap", function (a) {
    "entity" === a.type && (a.attributes.title = a.content.replace(/&amp;/, "\x26"))
});
...
Prism.languages.insertBefore("aspnet", "comment", {"asp comment": /<%--[\w\W]*?--%>/});
...
(function (a) {
    ...
    b = b.variable[1].inside;
    b["function"] = a.languages.bash["function"];
    b.keyword = a.languages.bash.keyword;
    b["boolean"] = a.languages.bash["boolean"];
    b.operator = a.languages.bash.operator;
    b.punctuation = a.languages.bash.punctuation
})(Prism);
...
(function (a) {
    var b = /#(?!\{).+/, d = {pattern: /#\{[^}]+\}/, alias: "variable"};
    a.languages.coffeescript = a.languages.extend("javascript", {
        comment: b,
        string: [/'(?:\\?[^\\])*?'/, {pattern: /"(?:\\?[^\\])*?"/, inside: {interpolation: d}}],
    ...
    a.languages.insertBefore("coffeescript", "comment", {
        "multiline-comment": {
            pattern: /###[\s\S]+?###/,
            alias: "comment"
        }, "block-regex": {pattern: /\/{3}[\s\S]*?\/{3}/, alias: "regex", inside: {comment: b, interpolation: d}}
    });
    a.languages.insertBefore("coffeescript", "string", {
        "inline-javascript": {
            pattern: /`(?:\\?[\s\S])*?`/,
            inside: {delimiter: {pattern: /^`|`$/, alias: "punctuation"}, rest: a.languages.javascript}
        }, "multiline-string": [{pattern: /'''[\s\S]*?'''/, alias: "string"}, {
            pattern: /"""[\s\S]*?"""/,
            alias: "string", inside: {interpolation: d}
        }]
    });
...
Prism.languages.fortran = {
    ...
    keyword: [/\b(?:INTEGER|REAL|DOUBLE ?PRECISION|COMPLEX|CHARACTER|LOGICAL)\b/i, /\b(?:END ?)?(?:BLOCK ?DATA|DO|FILE|FORALL|FUNCTION|IF|INTERFACE|MODULE(?! PROCEDURE)|PROGRAM|SELECT|SUBROUTINE|TYPE|WHERE)\b/i,
...
Prism.languages.perl = {
    ...
    regex: [/\b(?:m|qr)\s*([^a-zA-Z0-9\s\{\(\[<])(?:[^\\]|\\[\s\S])*?\1[msixpodualngc]*/,
        /\b(?:m|qr)\s+([a-zA-Z0-9])(?:[^\\]|\\.)*?\1[msixpodualngc]*/, /\b(?:m|qr)\s*\((?:[^()\\]|\\[\s\S])*\)[msixpodualngc]*/, /\b(?:m|qr)\s*\{(?:[^{}\\]|\\[\s\S])*\}[msixpodualngc]*/, /\b(?:m|qr)\s*\[(?:[^[\]\\]|\\[\s\S])*\][msixpodualngc]*/, /\b(?:m|qr)\s*<(?:[^<>\\]|\\[\s\S])*>[msixpodualngc]*/, {
...
Prism.languages.markup && (Prism.hooks.add("before-highlight", function (a) {
    ...
    if ("php" === a.language) {
        for (var b, d = 0; b = a.tokenStack[d]; d++)a.highlightedCode = a.highlightedCode.replace("{{{PHP" +
        (d + 1) + "}}}", Prism.highlight(b, a.grammar, "php").replace(/\$/g, "$$$$"));
        a.element.innerHTML = a.highlightedCode
    }
}), Prism.hooks.add("wrap", function (a) {
    "php" === a.language && "markup" === a.type && (a.content = a.content.replace(/(\{\{\{PHP[0-9]+\}\}\})/g, '\x3cspan class\x3d"token php"\x3e$1\x3c/span\x3e'))
...
(function (a) {
    a.languages.puppet = {
        ...
        comment: {pattern: /(^|[^\\])#.*/, lookbehind: !0},
        string: {
            pattern: /(["'])(?:\$\{(?:[^'"}]|(["'])(?:(?!\2)[^\\]|\\[\s\S])*\2)+\}|(?!\1)[^\\]|\\[\s\S])*\1/, inside: {
                "double-quoted": {
                    pattern: /^"[\s\S]*"$/,
                    inside: {}
                }
    ...
    var b = [{
        pattern: /(^|[^\\])\$\{(?:[^'"{}]|\{[^}]*\}|(["'])(?:(?!\2)[^\\]|\\[\s\S])*\2)+\}/, lookbehind: !0, inside: {
            "short-variable": {
                pattern: /(^\$\{)(?!\w+\()(?:::)?\w+(?:::\w+)*/,
                lookbehind: !0, alias: "variable", inside: {punctuation: /::/}
            }, delimiter: {pattern: /^\$/, alias: "variable"}, rest: a.util.clone(a.languages.puppet)
        }
...
(function () {
    "undefined" != typeof self && self.Prism && self.document && Prism.hooks.add("complete", function (a) {
        if (a.code) {
            var b = a.element.parentNode, d = /\s*\bline-numbers\b\s*/;
            if (b && /pre/i.test(b.nodeName) && !a.element.querySelector(".line-numbers-rows")) {
                d.test(a.element.className) && (a.element.className = a.element.className.replace(d, ""));
                d.test(b.className) || (b.className += " line-numbers");
